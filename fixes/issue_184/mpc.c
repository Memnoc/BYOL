      ...
      for (i = 0; i < st->parsers_num; i++) {
        if (st->parsers[i] && st->parsers[i]->name) {
          if (mpc_strcasecmp(st->parsers[i]->name, x) == 0) {
            return mpc_failf("Unknown Parser '%s'! Did you mean '%s'?", x,
                             st->parsers[i]->name);
          }
          if (strlen(msg) + strlen(st->parsers[i]->name) + 5 < 1024) {
            strcat(msg, "'");
            strcat(msg, st->parsers[i]->name);
            strcat(msg, "' ");
          }
        }
      }
      ...
      if (p == NULL) {
          ...
          if (st->parsers[j] && st->parsers[j]->name) {
            if (mpc_strcasecmp(st->parsers[j]->name, x) == 0) {
              ...
              if (st->error_msg == NULL) {
                st->error_msg =
                    malloc(strlen(x) + strlen(st->parsers[j]->name) + 50);
                sprintf(st->error_msg, "Uknown Parser '%s'! Did you mean '%s'?",
                        x, st->parsers[j]->name);
              }
              return mpc_failf("Unknown Parser '%s'! Did you mean '%s'?", x,
                               st->parsers[j]->name);
        ...
        if (st->error_msg == NULL) {
          if (strlen(msg) == 0) {
            ...
            sprintf(st->error_msg, "Uknown Parser '%s'!", x);
          } else {
            st->error_msg = malloc(strlen(x) + strlen(msg) + 40);
            sprintf(st->error_msg, "Uknown Parser '%s'! Available: %s", x, msg);
          }
        }
        if (strlen(msg) == 0) {
          return mpc_failf("Unknown Parser '%s'!", x);
...
static mpc_val_t *mpca_stmt_list_apply_to(mpc_val_t *x, void *s) {
  ...
  while (*stmts) {
    ...
    /* FIX: If we already found an error, just drain the remaining statements */
    if (res != NULL) {
      free(stmt->ident);
      free(stmt->name);
      mpc_soft_delete(stmt->grammar);
    ...
    /* FIX: If this specific rule failed, capture the error and continue
     * draining */
  ...
  if (!mpc_parse_input(i, Lang, &r)) {
    ...
    if (r.output) {
      ...
      if (out_parser->type == MPC_TYPE_FAIL) {
        /* DUPLICATE the message to prevent Segfault after mpc_delete */
